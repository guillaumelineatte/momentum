...
2. Deux environnements Vercel, deux bases Neon : **Production** (branche `production`) et **Preview** = version de test (branche `dev`). Chacun a sa propre `DATABASE_URL` et son propre `AUTH_SECRET`. Variables :
   ...
   - Pages légales : aucune variable. Le nom de l'éditeur est dans `lib/legal.ts` ; le contact affiché est la boîte e-mail de l'application (`SMTP_USER`).
...
- **Pages légales** : `/confidentialite` et `/mentions-legales` (modèles à faire relire ; identité de l'éditeur dans `lib/legal.ts`).
